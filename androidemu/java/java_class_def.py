...
class JavaClassDef(type):
    ...
    def find_field(cls, name, signature, is_static):
        ...
        try:
            if cls.jvm_super is not None:
                find = cls.jvm_super.find_field(name, signature, is_static)
                if find is not None:
                    return find
        except KeyError:
            pass
...
class java_lang_Object(metaclass=JavaClassDef, jvm_name='java.lang.Object'):

    def __init__(self):
        pass

    @java_method_def(name="getClass", signature="", native=False)
    def getClass(self):
        raise