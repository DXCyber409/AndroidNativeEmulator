...
class JNIEnv:
    ...
    def get_version(self, mu, env):
        logger.info("JNIEnv->GetVersion() was called.")
        return 0x00010004
    ...
    def find_class(self, mu, env, name_ptr):
        ...
        if clazz is None:
            ...
            gencode = """class %s(metaclass=JavaClassDef, jvm_name="%s"):\n\tdef __init__(self):\n\tpass
            """

            pclazz_name = name.replace('/', '_')
            gencode = gencode % (pclazz_name, name)

            raise RuntimeError('Could not find class \'%s\' for JNIEnv.\n%s' % (name, gencode))
    ...
    @native_method
    def get_superclass(self, mu, env, clazz_id):

        logger.info("GetSuperClass(%d)",clazz_id)
        clazz = self.get_reference(clazz_id)
        if clazz is None:
            RuntimeError("clazz not found!")

        super_cls = clazz.value.jvm_super
        if super_cls == None:
            RuntimeError("class(%s) not hava a super class" % clazz.value.jvm_name)
        return self.add_local_reference(jclass(super_cls))
    ...
    def push_local_frame(self, mu, env):
        logger.info("push_local_frame passed")
        pass

    @native_method
    def pop_local_frame(self, mu, env):
        logger.info("pop_local_frame passed")
        pass
    ...
    @native_method
    def get_object_class(self, mu, env, objJREF):
        obj = self.get_reference(objJREF)

        clazz = self._class_loader.find_class_by_id(obj.value.jvm_id)
        if clazz is None:
            # TODO: Proper Java error?
            raise RuntimeError('Could not find class \'%s\' for JNIEnv.' % objJREF)
    ...
    @native_method
    def call_int_method_v(self, mu, env, obj_idx, method_id):
        obj = self.get_reference(obj_idx)
        ...
        logger.debug("JNIEnv->CallIntMethodV(%s, %s <%s>) was called" % (
            obj.value.jvm_name,
            method.name,
            method.signature))
        ...
        return method.func(obj.value, self._emu, None)
    ...
    @native_method
    def call_nonvirtual_void_method_v(self, mu, env, obj_idx, clazz_idx, method_id, args):
        obj = self.get_reference(obj_idx)
        ...
        logger.debug("JNIEnv->CallNonvirtualVoidMethodV(%s, %s <%s>, 0x%x) was called" % (
            obj.value.jvm_name,
            method.name,
            method.signature, args))
    ...
    @native_method
    def set_object_field(self, mu, env, objJREF, field_id, valueJREF):

        obj = self.get_reference(objJREF)
        if not isinstance(obj, jobject):
            raise RuntimeError("Expect jclass.")
        ...
        logger.debug("JNIEnv->SetObjectField(%s, %s <%s>) was called" % (obj.value.jvm_name,
                                                                         field.name, field.signature))

        setattr(obj.value, field.name, valueJREF)
    ...
    def call_static_void_method_v(self, mu, env, clazz_idx, method_id, args):
        ...
        logger.debug("JNIEnv->CallStaticVoidMethodV(%s, %s <%s>, 0x%x) was called" % (
            clazz.value.jvm_name,
            method.name,
            method.signature, args))
    ...
    @native_method
    def set_static_object_field(self, mu, env, clazz_idx, field_id, ref):

        clazz = self.get_reference(clazz_idx)
        field = clazz.value.find_field_by_id(field_id)
        logger.debug("JNIEnv->SetStaticObjectField(%d, %d(%s),%d) was called" % (clazz_idx, field_id, field.name, ref))
        field.static_value = ref
    ...
    def new_string_utf(self, mu, env, bytes_ptr):
        ...
        return self.add_local_reference(jobject(String.java_lang_String(string)))
    ...
    def get_string_utf_chars(self, mu, env, string, is_copy_ptr):
        ...
        memory_helpers.write_utf8(mu, str_ptr, str_val.value)
    ...
    @native_method
    def new_object_array(self, mu, env, length, class_idx, init_element):
        logger.info("NewObjectArray(%d,%d,%d)", length, class_idx, init_element)
        #clazz = self.get_reference(class_idx)
        #if not isinstance(clazz, jclass):
        #    raise ValueError('Expected a jclass.')
        ...
        return self.add_local_reference(jobjectArray([None for i in range(length)]))
    ...
    @native_method
    def set_object_array_element(self, mu, env, arrary_ref, index, objref):
        logger.info("SetObjectArrayElement(%d,%d,%d)", arrary_ref, index, objref)

        arr = self.get_reference(arrary_ref).value
        arr[index] = objref
    ...
    @native_method
    def get_byte_array_elements(self, mu, env, idx, iscopy):
        logger.info("GetByteArrayElements was called.")
        ref = self.get_reference(idx)
        if not isinstance(ref, jbyteArray):
            raise RuntimeError("Expected a bytearray.")

        alloc_addr = self._emu.native_memory.allocate(len(ref.value))
        print (ref.value)
        memory_helpers.write_byte_array(mu, alloc_addr, ref.value)
        return alloc_addr
